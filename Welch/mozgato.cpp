...
public:
   ...
   */
	...
	//Ha nem egyezik a gyökérrel akkor rekurzívan lemásoljuk.
...
main (int argc, char *argv[])
{
    ...
    LZWBinFa binFa3;
    binFa3 = std::move(binFa);

    std::fstream kiFile4 ("4", std::ios_base::out);
    kiFile4 << binFa;
    kiFile4 << "depth = " << binFa.getMelyseg () << std::endl;
    kiFile4 << "mean = " << binFa.getAtlag () << std::endl;
    kiFile4 << "var = " << binFa.getSzoras () << std::endl;
    kiFile4.close();

    std::fstream kiFile2 ("2", std::ios_base::out);
    kiFile2 << binFa2;
    kiFile2 << "depth = " << binFa2.getMelyseg () << std::endl;
    kiFile2 << "mean = " << binFa2.getAtlag () << std::endl;
    kiFile2 << "var = " << binFa2.getSzoras () << std::endl;
    kiFile2.close();

    std::fstream kiFile3 ("3", std::ios_base::out);
    kiFile3 << binFa3;
    kiFile3.close();
